...
and which are to be blocked out (or black.)  This makes the word selection process easier, because
the word lengths, locations and orientations are now all determined.  What remains, then, is just a
...
massive tree of possibilities and backtracking when necessary.  The optimization trick is two fold.  First,
you should cache and hit that cache whenever possible so as not to duplciate work, such as determining
sets of words with various characteristics.  (I think this is called memoization.)  Second, there is
a really important way to cull branches of the search tree.
At any given point in the search, there is no point is going deeper into the tree if among the set of partially
completed word locations, any one of them has zero associated possibilities.  If you don't detect
this, then you may be descending the tree from this point in vain and wasting a lot of time.

And with that, the algorithm is surprising fast and successful.  At least, it has been in my testing so far.
I haven't yet run into a case where it gets stuck.  Before the optimizations, it was getting stuck all the time.
I can't be completely sure that what I have now is fool-proof, but it seems to work pretty well so far.